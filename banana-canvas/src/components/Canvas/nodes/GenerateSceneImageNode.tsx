...
import type { GenerateSceneImageSettings } from "../../../types/settings";

const RATIOS = ["16:9", "4:3", "1:1", "9:16", "2.35:1"];
const RESOLUTIONS = ["Auto", "1536x1024", "1024x1024"];

export const GenerateSceneImageNode = memo(function GenerateSceneImageNode({ id, selected }: NodeProps) {
  const theme = useUIStore((s) => s.theme);
  const isDark = theme === "dark";
  const { settings, updateSettings } = useNodeSettings<GenerateSceneImageSettings>(id);
  ...
  const handleGenerate = useCallback(async () => {
    setGenerating(true);
    const jobId = addJob({ id: uuid(), nodeId: id, type: "generate-scene-image", taskId: "", status: "running", progress: 0, createdAt: Date.now() });
    setTimeout(() => {
  ...
  return (
    <BaseNode id={id} type="generate-scene-image" selected={selected}>
