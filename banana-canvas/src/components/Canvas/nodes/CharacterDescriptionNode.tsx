...
import type { CharacterDescriptionSettings } from "../../../types/settings";

const DETAIL_LEVELS: CharacterDescriptionSettings["detailLevel"][] = ["brief", "detailed", "comprehensive"];
const DETAIL_LABELS: Record<CharacterDescriptionSettings["detailLevel"], string> = {
  brief: "简要", detailed: "详细", comprehensive: "全面",
};

export const CharacterDescriptionNode = memo(function CharacterDescriptionNode({ id, selected }: NodeProps) {
  const theme = useUIStore((s) => s.theme);
  const isDark = theme === "dark";
  const { settings, updateSettings } = useNodeSettings<CharacterDescriptionSettings>(id);
  ...
  const handleRun = useCallback(async () => {
    setRunning(true);
    const jobId = addJob({ id: uuid(), nodeId: id, type: "character-description", taskId: "", status: "running", progress: 0, createdAt: Date.now() });
    setTimeout(() => {
      const result = `角色描述 (${settings.detailLevel})`;
  ...
  return (
    <BaseNode id={id} type="character-description" selected={selected}>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select value={settings.detailLevel}
            onChange={(e) => updateSettings({ detailLevel: e.target.value as CharacterDescriptionSettings["detailLevel"] })}
            className="flex-1 text-[11px] px-1.5 py-1 rounded border outline-none" style={inputStyle}>
