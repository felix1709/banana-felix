...
export const GenerateSceneVideoNode = memo(function GenerateSceneVideoNode({ id, selected }: NodeProps) {
  ...
  const handleGenerate = useCallback(async () => {
    setGenerating(true);
    const jobId = addJob({ id: uuid(), nodeId: id, type: "generate-scene-video", taskId: "", status: "running", progress: 0, createdAt: Date.now() });
    setTimeout(() => {
    ...
    <BaseNode id={id} type="generate-scene-video" selected={selected}>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] w-14 shrink-0" style={{ color: isDark ? "#a1a1aa" : "#71717a" }}>分辨率</span>
        ...
        <input type="text" value={settings.cameraMotion}
          onChange={(e) => updateSettings({ cameraMotion: e.target.value })}
          placeholder="镜头运动描述"
